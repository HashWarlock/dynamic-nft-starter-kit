...
async function main() {
  ...
  const attestor = process.env['ZKEVM_PHALA_ORACLE_ATTESTOR'] || deployer.address;  // When deploy for real e2e test, change it to the real attestor wallet.
  ...
  const finalMessage = dedent`
    ...
    ZKEVM_CONSUMER_CONTRACT_ADDRESS=${consumer.address}
  `
  console.log(`\n${finalMessage}\n`);
